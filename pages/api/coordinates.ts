...
export default async function handler(
  ...
  if (method === 'GET') {
    try {
      ...
      // Fetch the image by its 'uniqueName'
      const image = await Coordinate.findOne({ uniqueName });
    ...
    try {
      ...
      // Update the image by 'uniqueName'
      const updatedCoordinate = await Coordinate.findOneAndUpdate(
        { uniqueName }, // Find the image by 'uniqueName'
    ...
    try {
      ...
      // Check if an image with the same uniqueName already exists
      const existingCoordinate = await Coordinate.findOne({ uniqueName });
      if (existingCoordinate) {
        return res.status(400).json({ message: 'An image with this uniqueName already exists' });
      }
